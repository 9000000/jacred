...
Страницы `/`, `/stats`, `/jobs` и `/settings` обслуживаются как SPA. Worker первым обрабатывает `/api/*`, JSON-маршруты `/stats/*`, `/health` и `/health/*` (включая `/health/background-jobs`), sync, Torznab, cron, dev и jsondb. Точный `/health` **не** покрывает `/health/background-jobs` — без glob `/health/*` Workers Assets отдадут `index.html`.
...
<Check>
  После deploy откройте `/health` и `/health/background-jobs` на домене Worker. Первый должен вернуть JSON `{ "status": "OK" }`, второй — JSON `{ "jobs": [...] }`, не HTML SPA.
</Check>
