...
Страницы `/`, `/stats` и `/settings` обслуживаются как SPA. Worker первым обрабатывает `/api/*`, JSON-маршруты `/stats/*`, `/health` и `/health/*` (включая `/health/background-jobs`), sync, Torznab, cron, dev и jsondb. Точный `/health` **не** покрывает `/health/background-jobs` — без glob `/health/*` Workers Assets отдадут `index.html`.
