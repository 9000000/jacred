...
<Check>
  После деплоя откройте `/stats` или `/jobs` напрямую (не с главной). Должна открыться SPA, а не 404 GitHub. Индикатор online в шапке должен ходить на `jacred.stream/health`.
</Check>
